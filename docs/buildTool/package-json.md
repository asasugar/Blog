...
使用`unpkg/jsdelivr`字段可以让 npm 上所有的文件都开启 cdn 服务，比如当我们从 CDN 访问使用 `Vue`时（ [https://unpkg.com/vue](https://unpkg.com/vue)），
...
此时查找 lock 文件依赖树，会发现 lock 文件全部被安装为我们指定的版本`graceful-fs@4.2.0`，当然你也可以针对单独的库进行版本重写，如下：
